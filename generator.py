...
EARLY_FORBIDDEN_RULES = [
    # 格式: (章节号, {关键词正则: "原因"})
    # 示例: (300, {"终极真相.*揭示": "第300章前严禁揭示终极真相"}),
]


# 需要检查一致性的关键词：出现时提醒人工确认
WATCH_KEYWORDS = [
    # 示例: "关键事件A", "核心道具C", "倒计时",
    # "春天的", "入夏", "入秋", "入冬",  # 季节变化
]
...
def run_consistency_check(chapter_text: str, chapter_num: int) -> list:
    ...
    # 则不再标记为🚫禁止，改为⚠️注意（因为可能是回忆/补充说明）。
    for chapter_limit, keyword_map in EARLY_FORBIDDEN_RULES:
        for kw, reason in keyword_map.items():
            for i, line in enumerate(lines, 1):
                if kw in line:
                    # 计算是否已过"安全期"：超过限制章节+10章后，改为⚠️注意而非🚫禁止
                    is_violation = chapter_num < (chapter_limit + 10)
                    issue_level = "🚫 禁止" if is_violation else "⚠️ 注意"
                    # 在🚫禁止时标注具体限制理由，在⚠️注意时标注已过限制期
    ...
    if char_count < 1500:
        issues.append({"level": "⚠️ 注意", "keyword": "字数过少", "line": 0,
                       "context": f"本章仅{char_count}字", "reason": "低于1500字下限"})
    elif char_count > 5000:
        issues.append({"level": "⚠️ 注意", "keyword": "字数过多", "line": 0,
...
def print_consistency_report(issues: list, chapter_num: int):
    """打印一致性检查报告"""
    if not issues:
        print(f"\n✅ 第{chapter_num}章一致性检查通过，未发现问题。")
        return

    forbidden = [i for i in issues if i["level"] == "🚫 禁止"]
    warnings = [i for i in issues if i["level"] == "⚠️ 注意"]

    print(f"\n{'='*50}")
    print(f"📋 第{chapter_num}章 一致性检查报告")
    print(f"{'='*50}")

    if forbidden:
        print(f"\n🚫 发现 {len(forbidden)} 个禁止项（必须修改）：")
        for item in forbidden:
            print(f"   L{item['line']}: [{item['keyword']}] {item['reason']}")
            print(f"         原文: {item['context']}")

    if warnings:
        print(f"\n⚠️ 发现 {len(warnings)} 个监控项（建议人工确认）：")
        for item in warnings:
            print(f"   [{item['keyword']}] {item['reason']}")

    if forbidden:
        print(f"\n❌ 建议：本章包含禁止关键词，请修改后再发布！")
    else:
        print(f"\n✅ 无禁止项，监控项请人工确认后即可发布。")
    print(f"{'='*50}")
...
def get_latest_chapter_info() -> tuple:
    """自动计算当前系统的卷号和最新章号。
    P1 修复：扫描实际输出目录找到最高卷号，不再盲信 config['current_volume']。
    """
    import re as _re
    out_dir = DIRS["out"]
    best_vol = 0
    ...
    if os.path.isdir(out_dir):
        for entry in os.listdir(out_dir):
            vol_path = os.path.join(out_dir, entry)
            if not os.path.isdir(vol_path):
                continue
            m = _re.match(r'^第(\d+)卷$', entry)
            if not m:
                continue
            vol_num = int(m.group(1))
            # 只认包含章节文件的卷目录
            chap_files = glob.glob(os.path.join(vol_path, "第*.txt"))
            if chap_files and vol_num > best_vol:
                best_vol = vol_num
    
    # 如果没有找到任何含章节的卷目录，使用 config 中的值
    if best_vol == 0:
        best_vol = config.get("current_volume", 1)
    
    # 同步 config（防止后续逻辑读 config 时不一致）
    config["current_volume"] = best_vol
...
def build_system_prompt() -> str:
    ...
    prompt_parts.append(
        ...
        "3. 如当前写作要求与既有设定冲突，以【唯一真相设定/时间线锚点/当前卷大纲/全局备忘录/伏笔表】为最高优先级；当前写作要求只能补充表现方式，不能推翻既有真相。\n"
        "4. 禁止把一句完整意思拆成多行短句来强行制造电影感；章尾悬念最多保留1-2个短锤句，其余必须用正常叙述落地。\n"
        "5. 慎用'某种''像是''不是……是……'和破折号——硬造神秘感，优先用动作、细节、场景推进压迫感。\n"
        "6. 【去AI味黑名单】：绝对禁止使用'不可置信地'、'不由得'、'深吸一口气'、'倒吸一口凉气'、'嘴角微勾/勾起一抹弧度'、'深邃/狭长/漆黑的眼眸'、'空气仿佛凝固了'、'陷入了死寂'。这些词极度油腻且有机器感，请直接描写具体动作或白描。\n"
        "7. 【禁止模板结尾】：章尾绝不能出现'而她，还一无所知'、'命运的齿轮开始转动'、'一切才刚刚开始'等模板化强行留悬念的套话。\n"
        "8. 【心理描写与配角反应】：女主的心声必须有“留白”，不要一句话把人设+秘密+反差全盘托出，要分层次吐槽。配角的震惊反应必须多样化，绝对禁止重复使用“僵住→震惊→她怎么知道”的单一模式，禁止频繁出现“她怎么会知道”这种低级独白。\n"
        "9. 【视角与旁白限制】：禁止旁白强行下结论（如“故作镇定”、“不受控制”、“眼神变得深沉”），必须通过具体的肢体动作和对话让读者自己体会。女主的内心戏要符合17岁高中生的特质，不要过度理智和全知全能。\n"
        "10. 【系统存在感】：系统除关键剧情节点外尽量保持静默，绝对禁止频繁出现“叮！”的机械音打断阅读节奏。\n"
        "11. 以下是你的全部记忆库，请严格遵循相应的设定标签。\n"
    )

    # 2. 读取各维度记忆层级（防崩盘多级记忆塔）
    # Layer 2: 长期历史记忆 (卷宗总结)
    # Layer 0: 唯一真相设定表（最高优先级硬约束）
    canon_table = read_text_safe(os.path.join(DIRS["plot"], "唯一真相设定表.md"))
    if canon_table:
        prompt_parts.append(
            "<canonical_settings_table>\n"
            "【最高优先级】以下是唯一真相设定表，任何章节内容都不得与此表矛盾。\n"
            "如果你写的内容与此表冲突，请以此表为准。\n\n"
            f"{canon_table}\n"
            "</canonical_settings_table>\n"
        )
    ...
    while True:
        ...
        if choice == '1':
            ...
            single_chapter_rules = (
                ...
                "禁止把一句完整意思拆成3-5个独立短段制造假节奏，章尾不要整段都写成短句锤子。\n"
                "</chapter_quality_rules>\n"
            )
            ...
            while True:
                ...
                if content:
                    ...
                    if action == '1':
                        ...
                        # 自动一致性检查
                        issues = run_consistency_check(content, chap_num)
                        print_consistency_report(issues, chap_num)
