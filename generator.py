...
DIRS = {
    ...
    "hist":  os.path.join(SCRIPT_DIR, "history"),
    "logs":  os.path.join(SCRIPT_DIR, "logs"),
    "publish": os.path.join(SCRIPT_DIR, "publish"),
}
...
def build_system_prompt() -> str:
    ...
    prompt_parts.append(
        ...
        "8. 【人物反应】：角色反应必须多样化，禁止反复使用同一种震惊、沉默、僵住、倒吸气模板。\n"
        "9. 【视角与旁白限制】：禁止旁白强行下结论，优先通过动作、对话、场景细节让读者自己体会。\n"
        "10. 【题材差异】：具体题材规则以项目的基调铁律、唯一真相设定表和 tool_rules.json 为准，不得自行套用旧项目设定。\n"
