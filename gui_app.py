...
class NovelGeneratorGUI:
    ...
    def __init__(self, root):
        ...
        if last_dir and os.path.isdir(last_dir):
            ...
            messagebox.showinfo("欢迎使用！",
                                "系统已载入当前工具目录作为默认新书项目。\n\n"
                                "请先点击「开书向导」补齐书名、总章数和卷范围，再到「⚙ 设置」填入 API Key。")
    ...
    def _default_project_config(self):
        return {
            ...
            "chapter_char_target_max": 5500,
            "trusteeship_policy": "lenient_continue",
            "auto_outline_from_volume": False,
    ...
    def _get_chapter_char_limits(self):
        return {
            ...
            "target_max": int(self.config.get("chapter_char_target_max", 5500) or 5500),
        }

    def _is_lenient_policy(self):
        return self.config.get("trusteeship_policy", "lenient_continue") == "lenient_continue"
    ...
    def switch_project(self, folder):
        ...
        self.root.title(f"通用小说一键托管工具 - {os.path.basename(folder)}")
    ...
    def _load_project_config(self, folder):
        ...
        if os.path.exists(pconfig_path):
            try:
                ...
                # 隔离字段同步
                for key in (
                    "model", "temperature", "current_volume", "enabled_skills", "tone_rules",
                    "book_title", "genre_template", "target_total_chapters", "volume_ranges",
                    "chapter_char_min", "chapter_char_target_min", "chapter_char_target_max",
                    "trusteeship_policy", "auto_outline_from_volume", "health_check_future_chapters",
                ):
                    if key in pconfig:
    ...
    def create_widgets(self):
        ...
        ttk.Button(toolbar, text="🧭 开书向导", command=self.book_wizard).pack(side=tk.LEFT, padx=3)
        ttk.Button(toolbar, text="🩻 体检大纲", command=self.outline_audit).pack(side=tk.LEFT, padx=3)
        ...
        self.btn_full_book = ttk.Button(btn_frame, text="📚 一键写完整本", command=self.batch_generate_full_book)
        self.btn_full_book.pack(side=tk.LEFT, padx=3)
    ...
    def open_settings(self):
        ...
        model_entry.insert(0, self.config.get("model", "deepseek-v4-pro"))
    ...
    def _auto_generate_outline(self, chap_num, prev_content):
        ...
        sys_prompt = (
            "你是专业小说细纲规划师。根据卷大纲、备忘录和上一章结尾，"
            "为指定章节写出一段150-300字的逐章细纲。\n"
            "细纲格式：核心事件 + 出场人物 + 情绪/爽点 + 章末钩子方向。\n"
            "只输出细纲文本，不要Markdown标题，不要标注章节号。"
        )
...
请输出第{chap_num}章细纲："""
        ...
        try:
            ...
            self._ui_progress_append(f"    ❌ 自动展开失败: {str(e)[:60]}\n")
            return ""
    ...
    def disable_buttons(self):
        for btn in [self.btn_new, self.btn_continue, self.btn_save_new, self.btn_save_append, self.btn_batch, self.btn_full_book]:
    ...
    def enable_buttons(self, is_new=True):
        ...
        self.btn_full_book.config(state=tk.NORMAL)
        if is_new:
    ...
    def _check_content_drift(self, content):
        ...
        hits = [kw for kw in banned_keywords if kw in content]
        # 也检测 Markdown 残留
        md_patterns = [
            r'\*\*.+?\*\*', r'__.+?__', r'^\s*#{1,6}\s+',
            r'`[^`]+`',
    ...
    def book_wizard(self):
        title = simpledialog.askstring("开书向导", "书名：", initialvalue=self.config.get("book_title", "我的新小说"))
        if not title:
            return
        genre = simpledialog.askstring("开书向导", "题材模板/类型：", initialvalue=self.config.get("genre_template", "通用空白"))
        if genre is None:
            return
        total = simpledialog.askinteger("开书向导", "目标总章数：", initialvalue=int(self.config.get("target_total_chapters", 300) or 300), minvalue=1)
        if not total:
            return
        volume_size = simpledialog.askinteger("开书向导", "每卷默认章数：", initialvalue=50, minvalue=1)
        if not volume_size:
            return
        ...
        plot_dir = generator.DIRS["plot"]
        self._ensure_text_file(os.path.join(plot_dir, "全书大纲.txt"), f"《{title}》全书大纲\n\n请填写：核心卖点、主线、分卷结构、结局方向。\n")
        self._ensure_text_file(os.path.join(plot_dir, "当前卷大纲.txt"), "第一卷\n\n请填写本卷主线、阶段目标、关键冲突和卷末落点。\n")
        self._ensure_text_file(os.path.join(plot_dir, "第一卷逐章细纲.txt"), "第1章 标题待定\n\n核心事件：\n- 请填写本章必须发生的剧情。\n\n章末悬念：请填写。\n")
        self._ensure_text_file(os.path.join(plot_dir, "基调铁律.txt"), "请填写本书不可违反的文风、题材、角色和平台规则。\n")
        self._ensure_text_file(os.path.join(plot_dir, "唯一真相设定表.md"), "# 唯一真相设定表\n\n请填写客观真相、角色误判、禁止提前揭示项。\n")
        self._ensure_text_file(os.path.join(plot_dir, "时间线锚点.txt"), "请填写关键事件唯一发生章号，防止回档和重复。\n")
        self._ensure_text_file(os.path.join(plot_dir, "伏笔与因果追踪表.txt"), "请填写伏笔、首次出现章、回收计划。\n")
        self._ensure_text_file(os.path.join(plot_dir, "关键词一致性检查表.txt"), "请填写必须保持唯一解释的专有名词。\n")
        self._ensure_text_file(os.path.join(plot_dir, "每10章审稿清单.txt"), "每10章复核：时间线、角色状态、伏笔、真相揭示、重复事件。\n")
        ...
        self.refresh_knowledge_base()
        self.refresh_status()
        messagebox.showinfo("开书向导", "新书基础配置和空白模板已准备好。")
    ...
    def batch_generate_full_book(self):
        ...
        if not planned_end or planned_end < next_chap:
            messagebox.showinfo("一键写完整本", "未找到有效终章规划，请先在开书向导或 project_config.json 中设置 target_total_chapters。")
            return
        count = planned_end - next_chap + 1
        if count <= 0:
            messagebox.showinfo("一键写完整本", "已经写到规划终章。")
            return
        issues = self._run_health_check_internal(future_window=count)
        fail_items = [(desc, st) for desc, st in issues if st == "FAIL"]
        if fail_items:
            fail_detail = "\n".join(f"  ❌ {desc}" for desc, _ in fail_items[:20])
            messagebox.showerror("一键写完整本被阻止", f"体检发现 {len(fail_items)} 项 FAIL，必须先修复：\n\n{fail_detail}")
            return
        if not messagebox.askyesno("一键写完整本", f"将从第 {next_chap} 章写到第 {planned_end} 章，共 {count} 章。是否开始？"):
            return
        self._start_batch_job(count, label="一键写完整本")

    def _start_batch_job(self, count, label="批量生成"):
        self._stop_event.clear()
        self.is_batch_running = True
        self._current_batch_job_id = datetime.now().strftime("batch_%Y%m%d_%H%M%S")
        self._write_batch_state(
            job_id=self._current_batch_job_id,
            status="running",
            planned_count=count,
            completed_count=0,
            skipped_chapters=[],
        ...
        self._append_batch_audit({"event": "batch_start", "label": label, "planned_count": count})
        self._ensure_batch_log_window()
        self._ui_progress_append(f"🚀 {label}启动，共计划 {count} 章。\n\n", clear=True)
        self.disable_buttons()
        ...
        for i in range(total_count):
            ...
            if not chapter_outline:
                if not self.config.get("auto_outline_from_volume", False):
                    self._ui_progress_append(f"  ⏸ 未找到第 {chap_num} 章逐章细纲，且自动补细纲未开启。\n")
                    skipped_chapters.append(chap_num)
                    batch_end_message = f"⏸ 批量生成已暂停：第 {chap_num} 章缺少逐章细纲。"
                    break
                self._ui_progress_append(f"  🤖 无细纲，从卷大纲自动展开...\n")
                chapter_outline = self._auto_generate_outline(chap_num, prev_content)
                if not chapter_outline:
                    self._ui_progress_append(f"  ❌ 自动展开失败，跳过第{chap_num}章\n")
                    skipped_chapters.append(chap_num)
                    batch_end_message = f"⏸ 批量生成已暂停：第 {chap_num} 章无法自动生成细纲。"
                    break
            outline_guard = self._run_outline_reveal_guard(chapter_outline, chap_num)
            if outline_guard["status"] == "FAIL":
                self._ui_progress_append(
                    f"  ⚠ 第 {chap_num} 章细纲触发真相护栏，已保存待审草稿并暂停。\n"
                    f"    🔒 {outline_guard['summary']}\n"
                )
                _save_review_draft(
                    chapter_filepath,
                    f"第{chap_num}章 [待重写]\n\n本章因细纲触发真相护栏被拦截，需人工修复细纲后重新生成。\n"
                )
                skipped_chapters.append(chap_num)
                batch_end_message = f"⏸ 批量生成已暂停：第 {chap_num} 章细纲未通过真相护栏。"
                break
            elif outline_guard["status"] == "WARN":
                reveal_rules = self._load_reveal_rules()
                if reveal_rules and reveal_rules.get("strict_mode", False):
                    self._ui_progress_append(
                        f"  ⚠ 第 {chap_num} 章细纲触发真相护栏（严格模式），已保存待审草稿并暂停。\n"
                        f"    🔒 {outline_guard['summary']}\n"
                    )
                    _save_review_draft(
                        chapter_filepath,
...
{prev_content if prev_content else "（这是第一章，无上文）"}"""
            ...
            # 托管策略：初稿 + 两次带诊断重写；短章需要更强约束，避免反复停在字数线。
            max_retries = 3
            retry_count = 0
            chapter_content = ""
            ...
            def _retry_or_block(reason, content):
                nonlocal retry_count, blocked_by_quality_gate, _quality_gate_skip_content, last_failure_reason
                last_failure_reason = reason
                if retry_count < max_retries - 1 and not self._stop_event.is_set():
                    repair_notes.append(f"第{retry_count + 1}版未通过：{reason[:500]}")
                    retry_count += 1
                    self._write_batch_state(
                        status="running",
                        current_chapter=chap_num,
                    ...
                    self._ui_progress_append(
                        f"  🔁 {reason[:120]}，自动重写第 {retry_count + 1}/{max_retries} 版...\n"
                    )
                    return True

                blocked_by_quality_gate = True
                _quality_gate_skip_content = content
            ...
            while retry_count < max_retries and not success and not self._stop_event.is_set():
                try:
                    ...
                    if repair_notes:
                        extra_repair_rule = ""
                        if any("字数不足" in note for note in repair_notes[-3:]):
                            extra_repair_rule = (
                                "\n【字数不足专项返修】\n"
                                f"- 上一版低于硬下限，本版必须写到{char_limits['target_min']}-{min(char_limits['target_max'], char_limits['target_min'] + 1000)}个中文字符；宁可多写，不可低于{char_limits['min']}。\n"
                                "- 不要用摘要跳过情绪和动作，必须补足至少3个完整小场景：开场承接、人物互动、章尾落点。\n"
                                "- 每个小场景都要有动作、对话、环境细节和主角心理变化，不能只用内心独白凑字。\n"
                            )
                        attempt_prompt = (
                            plan_prompt
                            + "\n\n【自动返修要求】\n"
                            + "\n".join(repair_notes[-3:])
                            + extra_repair_rule
                            + "\n请不要局部修补上一版，请基于同一细纲完整重写本章。"
                            + "必须修复上述问题，同时保持章节号、核心事件和上一章衔接不变。"
                        )
                    self._write_batch_state(
                        status="running",
                        current_chapter=chap_num,
                    ...
                    # 超长截断: 超过目标上限时按段落边界截到目标范围内
                    long_limit = max(char_limits["target_max"] + 300, char_limits["target_max"])
                    if chinese_chars > long_limit:
                        chapter_content = self._truncate_chapter(chapter_content, max(char_limits["target_min"], char_limits["target_max"] - 200))
                        chinese_chars = len(re.findall(r'[\u4e00-\u9fff]', chapter_content))
                    ...
                    if drift_hits:
                        ...
                        self._ui_progress_append(f"  🛡️ {reason}\n")
                        _retry_or_block(reason, chapter_content)
                        continue
                    ...
                    if release_guard["status"] == "FAIL":
                        ...
                        self._ui_progress_append(f"  🧱 设定拦截：{release_guard['summary']}\n")
                        if self._is_lenient_policy():
                            chapter_status = "可继续但需复核"
                            chapter_review_notes.append(reason)
                            self._append_batch_audit({
                                "event": "release_guard_warn_continue",
                                "chapter": chap_num,
                                "reason": reason[:800],
                            })
                        else:
                    ...
                    if release_guard["status"] == "WARN":
                        ...
                        self._ui_progress_append(f"  ⚠ 设定预警：{release_guard['summary']}\n")

                    # ---- 真相分级放行检查 (FAIL 直接存草稿跳过) ----
                    ...
                    if truth_guard["status"] == "FAIL":
                        ...
                        reveal_rules = self._load_reveal_rules()
                        if self._is_lenient_policy() and not (reveal_rules and reveal_rules.get("strict_mode", False)):
                            chapter_status = "可继续但需复核"
                            chapter_review_notes.append(reason)
                            self._append_batch_audit({
                                "event": "truth_guard_warn_continue",
                                "chapter": chap_num,
                                "reason": reason[:800],
                            })
                        else:
            ...
            if not success:
                ...
                if blocked_by_quality_gate:
                    ...
                    if save_content:
                        ...
                        self._ui_progress_append(
                            f"\n  ⚠ 第 {chap_num} 章质检未通过，已保存待审草稿({chinese_chars}字)：{os.path.basename(draft_path)}\n"
                            f"  ⏸ 批量生成已暂停。\n\n"
                        )
                        skipped_chapters.append(chap_num)
                        batch_end_message = f"⏸ 批量生成已暂停：第 {chap_num} 章质检未通过。"
                        break
                    skipped_chapters.append(chap_num)
                    batch_end_message = f"⏸ 批量生成已暂停：第 {chap_num} 章未能生成可审草稿。"
                    break
                ...
                if chapter_content:
                    ...
                    chinese_chars = len(re.findall(r'[\u4e00-\u9fff]', chapter_content))
                    self._ui_progress_append(
                        f"\n  ⚠ 第 {chap_num} 章未通过，已保存待审草稿({chinese_chars}字)：{os.path.basename(draft_path)}\n"
                        f"  ⏸ 批量生成已暂停。\n\n"
                    )
                    skipped_chapters.append(chap_num)
                    batch_end_message = f"⏸ 批量生成已暂停：第 {chap_num} 章未通过。"
                    break
                skipped_chapters.append(chap_num)
                batch_end_message = f"⏸ 批量生成已暂停：第 {chap_num} 章未能生成。"
                break
            ...
            try:
                if chap_num % 10 == 0:
                    temp_current_path = os.path.splitext(chapter_filepath)[0] + "_托管检查.txt"
                    with open(temp_current_path, "w", encoding="utf-8") as f:
                        f.write(chapter_content)
                    scan_path = temp_current_path
                else:
                    scan_path = chapter_filepath
                cross_issues = self._run_cross_chapter_check(chapter_content, chap_num, scan_path)
            finally:
                if temp_current_path and os.path.exists(temp_current_path):
                    try:
            ...
            if historical_cross_issues:
                self._ui_progress_append(
                    f"  ℹ 跨章扫描发现 {len(historical_cross_issues)} 个历史区间问题，"
                    f"不归因到第 {chap_num} 章，不中断托管。\n"
                )
                self._append_batch_audit({
                    "event": "historical_cross_issues_ignored",
                    "chapter": chap_num,
                    "issues": historical_cross_issues[:5],
                })
            high_cross_issues = [i for i in current_cross_issues if i.get('severity') == 'HIGH']
            if high_cross_issues:
                draft_path = _save_review_draft(chapter_filepath, chapter_content)
                skipped_chapters.append(chap_num)
                self._ui_progress_append(
                    f"  ⏸ 第 {chap_num} 章跨章扫描发现高优先级问题，已保存待审草稿：{os.path.basename(draft_path)}\n"
                    f"  批量生成已暂停。\n"
                )
                self._write_batch_state(
                    status="paused",
                    ...
                    message=f"跨章扫描发现{len(high_cross_issues)}个高优先级问题",
                )
                self._append_batch_audit({
                    "event": "chapter_paused",
                    "chapter": chap_num,
                    "reason": "跨章扫描未通过",
                    "issues": high_cross_issues[:5],
                })
                self._ui(self.refresh_reader_files_silent)
                batch_end_message = f"⏸ 批量生成已暂停：第 {chap_num} 章跨章扫描未通过。"
                break
            ...
            if chapter_status == "正式可用":
                publish_path = self._copy_to_publish(chapter_filepath, chap_num)
                publish_msg = f"；已同步发布稿 {os.path.basename(publish_path)}" if publish_path else ""
                self._ui_progress_append(f"  ✅ 第 {chap_num} 章已保存！状态：{chapter_status}({chinese_chars}字){publish_msg}\n")
            else:
                self._ui_progress_append(f"  ⚠ 第 {chap_num} 章已保存！状态：{chapter_status}({chinese_chars}字)，需后续复核。\n")
            self._record_chapter_status(chap_num, chapter_status, chapter_review_notes, chapter_filepath, chinese_chars)
            self._write_batch_state(
                status="running",
                current_chapter=chap_num,
                completed_count=i + 1,
            ...
            # 自动完结检测
            ending_markers = ["（全书完）", "（完）", "全书完", "全剧终", "—— 全文完 ——", "（大结局）", "the end", "【完结】", "（全文完）"]
        ...
        self._ui_progress_append(f"\n{'=' * 40}\n{batch_end_message}\n")
        final_status = "completed" if not skipped_chapters and "完毕" in batch_end_message else "paused"
    ...
    def _dedup_chapter(self, content):
        ...
        if new_chars >= self._get_chapter_char_limits()["min"]:
    ...
    def _detect_chapter_repetition(self, content, chap_num=None):
        ...
        if repeated_count >= 3 or repeated_chars >= 360:
    ...
    def _record_chapter_status(self, chap_num, status, notes, chapter_filepath, chars):
        payload = {
            "chapter": chap_num,
            "status": status,
            "notes": notes or [],
        ...
        try:
            os.makedirs(generator.DIRS.get("logs", generator.DIRS["out"]), exist_ok=True)
            with open(os.path.join(generator.DIRS.get("logs", generator.DIRS["out"]), "chapter_status.jsonl"), "a", encoding="utf-8") as f:
    ...
    def _copy_to_publish(self, chapter_filepath, chap_num):
        try:
            ...
            shutil.copy2(chapter_filepath, publish_path)
            return publish_path
        except Exception:
            return ""
        ...
        for skill_id in pipeline_skills:
            ...
            if output_type == "save_to_file":
                ...
                if save_filename == "全局备忘录.txt":
                    ...
                    if not memory_ok:
                        self._ui_progress_append(f" ⚠ {memory_msg}，保留原备忘录")
                        self._append_batch_audit({
...
【第 {self.latest_chap} 章最新内容】：{text}"""
    ...
    def export_book(self):
        publish_dir = generator.DIRS.get("publish", "")
        out_dir = publish_dir if publish_dir and os.path.isdir(publish_dir) and glob.glob(os.path.join(publish_dir, "第*.txt")) else generator.DIRS.get("out", "output")
        if not os.path.exists(out_dir):
    ...
    def _run_health_check_internal(self, future_window=None):
        ...
        # API Key 检查
        active_model = self.model_var.get() if hasattr(self, "model_var") else ""
        for preset_name, preset in MODEL_PRESETS.items():
            key_field = preset.get("config_key_field", "api_key")
            key_value = (self.config.get(key_field) or "").strip()
            if key_value and key_value not in {"YOUR_DEEPSEEK_API_KEY_HERE", "YOUR_MINIMAX_API_KEY_HERE"}:
                results.append((f"{preset_name} API Key", "PASS"))
            elif preset_name == active_model:
                results.append((f"{preset_name} API Key", "FAIL"))
            else:
                results.append((f"{preset_name} API Key", "WARN"))
        ...
        # 托管连续性检查：待审稿会污染后续自动生成，必须先处理。
        pending_reviews = []
        out_dir = generator.DIRS["out"]
        if os.path.isdir(out_dir):
            for root_dir, _, fnames in os.walk(out_dir):
                if ".backup" in root_dir.split(os.sep):
                    continue
                for fname in fnames:
                    if fname.endswith("_待审.txt"):
                        pending_reviews.append(os.path.join(root_dir, fname))
        if pending_reviews:
            results.append((f"待审稿未处理({len(pending_reviews)}个)", "FAIL"))
        else:
