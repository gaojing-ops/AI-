...
class SimpleLocalRAG:
    ...
    def _idf(self, token):
        """计算平滑 IDF，统一用于 query 和 document。"""
        df = self.doc_freqs.get(token, 0)
        return math.log((self.total_docs + 1) / (df + 1)) + 1

    def _vector_norm(self, tf, length):
        """按当前语料库状态计算向量范数，避免新增文档后旧缓存失真。"""
        if length <= 0:
            return 0
        norm = 0
        for token, count in tf.items():
            ...
            norm += weight ** 2
        return math.sqrt(norm) if norm > 0 else 0
    ...
    def search(self, query, top_k=3, threshold=0.01):
        ...
        for doc_id, doc in self.documents.items():
            doc_tf = doc["tf"]
            doc_len = doc["length"]
            doc_norm = self._vector_norm(doc_tf, doc_len)
