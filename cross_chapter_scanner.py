...
def _load_project_tool_rules(project_dir=None):
    global EVENT_PATTERNS, EVENT_COMPLETION_CHAPTERS, FORBIDDEN_TERMS, CHAPTER_GATED_TERMS, OLD_NAME_PATTERNS, ALLOWED_CONTEXTS
    EVENT_PATTERNS = []
    EVENT_COMPLETION_CHAPTERS = {}
    FORBIDDEN_TERMS = []
    CHAPTER_GATED_TERMS = []
...
def get_chapter_files(volume_dir, start=None, end=None):
    ...
    files = [
        ...
        if "_待审" not in os.path.basename(f)
    ]
